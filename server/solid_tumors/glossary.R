server_solid_tumors_glossary <- function(input, output, server) {
  output$solid_glossary <- DT::renderDataTable({
    ...
    solid_meta2 <- solid_meta[solid_meta$PRoXe_Column_Header %in% names(solid), ]
    if (!all(solid_meta$PRoXe_Column_Header %in% names(solid))) warning("not all solid metadata columns are in solid Database Explorer, and Glossary will be incomplete.")
    names(solid_meta2) <- gsub("_", " ", names(solid_meta2))
    solid_meta2[, c("PRoXe Column Header", "Column Description")]
  },
  filter = "top",
